        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-3">
                    <h5 class="card-title mb-0">Laporan Mingguan - {{ \Carbon\Carbon::parse($startDate)->format('d M Y') }} s.d {{ \Carbon\Carbon::parse($endDate)->format('d M Y') }}</h5>
                    <div class="d-flex gap-2">
                        <a href="{{ route('reports.export.pdf') }}?start_date={{ $startDate }}&end_date={{ $endDate }}" class="btn btn-danger">
                            <i class="fas fa-file-pdf"></i> Export PDF
                        </a>
                        <a href="{{ route('reports.export.excel') }}?start_date={{ $startDate }}&end_date={{ $endDate }}" class="btn btn-success">
                                    ...
                                    @php
                                        ...
                                        foreach($employeeAttendances as $attendance) {
                                            if($attendance->check_in_time && $attendance->check_out_time) {
                                                $totalHours += $attendance->check_in_time->diffInHours($attendance->check_out_time);
                                            }
                                        }
